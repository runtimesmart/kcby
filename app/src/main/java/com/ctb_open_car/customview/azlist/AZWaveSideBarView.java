package com.ctb_open_car.customview.azlist;
...
public class AZWaveSideBarView extends View {
	...
	private void drawSelect(Canvas canvas) {
		if (mSelect >= 0 && mSelect < mLetters.size()) {
	...
	private void drawHint(Canvas canvas) {
		...
		// 绘制提示字符
		if (mAnimationRatio >= 0.9f && mSelect >= 0 && mSelect < mLetters.size()) {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				if (y < mSlideBarRect.top || y > mSlideBarRect.bottom) {
					return false;
				}
				mTouchY = (int) y;
